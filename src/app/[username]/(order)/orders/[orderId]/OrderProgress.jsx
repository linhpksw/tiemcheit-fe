...
const OrderProgress = ({ orderId, status, refresh, isValid }) => {
	...
	const updateStatus = async (option) => {
		try {
			...
			console.log(baseURL);
			const response = await robustFetch(baseURL, 'PATCH', 'Cập nhật thành công', null);
			refresh();
		} catch (err) {
	...
	const handleStatusClick = (index) => {
		if (isValid && (index === currentStatusIndex + 1 || index <= currentStatusIndex)) {
						...
						index <= 3 && (
							<div key={index} className='flex flex-col items-center justify-center'>
								<div
									...
									style={{
										cursor: isValid && index === currentStatusIndex + 1 ? 'pointer' : 'default',
									}}>

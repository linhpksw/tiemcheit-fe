    ...
    <div className="overflow-hidden rounded-lg border border-default-200">
      <div className="border-b border-b-default-200 p-6">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <DemoFilterDropdown
            filterOptions={["Best Seller", "High to Low", "Low to High"]}
            filterType="Popular"
          />
          <DemoFilterDropdown
          ...
          />
          <DemoFilterDropdown
            filterOptions={["Average", "Good", "Best"]}
            filterType="Rating"
          />
                          ...
                          if (column.key == "name") {
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center justify-start gap-1">
                                  {Array.from(
                                    new Array(Math.floor(row.review.stars ?? 0))
                                  ).map((_star, idx) => (
                                    <FaStar
                                      key={idx}
                                      size={18}
                                  ...
                                  {!Number.isInteger(row.review.stars ?? 0) && (
                                    <FaStarHalfStroke
                                      size={18}
                                  ...
                                  {(row.review.stars ?? 0) < 5 &&
                                    Array.from(
                                      new Array(
                                        5 - Math.ceil(row.review.stars ?? 0)
                                      )
                                    ).map((_val, idx) => (
                                      <FaStar
                                        key={idx}
                            ...
                            return (
                              ...
                              >
                                ...
                                {typeof tableData == "object" && row.category
                                  ? row.category.name
                                  : tableData}
                              </td>
                            );

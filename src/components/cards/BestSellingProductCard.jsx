		...
		<Link
			...
			<div className='relative divide-y divide-default-200 overflow-hidden rounded-lg'>
				<div className='mx-auto mb-4'>
					<Image src={getImagePath(image)} width={204} height={159} className='h-full w-full' alt={name} />
				</div>
				<div className='pt-2'>
					<h4 className='mb-2 line-clamp-1 text-xl font-semibold text-default-800'>{name}</h4>
					<h6 className='text-lg font-semibold text-default-500'>{formatCurrency(discountedPrice)}</h6>
				</div>
			</div>
		</Link>
	);
